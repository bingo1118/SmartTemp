...
public class TimePickerViewHelper extends LinearLayout{
    ...
    public TimePickerViewHelper(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        if (typedArray != null) {
            ...
            hasDate=typedArray.getBoolean(R.styleable.TimePickerViewHelper_timerpicker_date,true);
            if(hasDate){
        ...
        pvTime = new TimePickerBuilder(context, new OnTimeSelectListener() {
            ...
            public void onTimeSelect(Date date, View v) {
                if(mOnTimeGetListener!=null){
                    DateFormat dFormat ;
                    if(hasDate){
                        dFormat=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss"); //HH表示24小时制；
                    }else{
                        dFormat=new SimpleDateFormat("HH:mm:ss"); //HH表示24小时制；
                    }
